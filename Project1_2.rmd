...
daily <- read_csv("natural_gas_germany_daily.csv")

# Read the second CSV file, ensuring the Date column is correctly parsed
gas <- read_csv("dutch_ttf_natural_gas.csv", col_types = cols(Date = col_date(format = "%Y-%m-%d")))

# Display the number of rows and columns in each dataset
cat("Dimensions of daily:", dim(daily), "\n")
cat("Dimensions of gas:", dim(gas), "\n")
```
...
```{r}
# Summaries of typical values for the three types of consumption
...
# Ensuring 'date' column is of Date type
daily$date <- as.Date(daily$date)

# Calculating number of weeks
num_weeks <- as.numeric(difftime(max(daily$date), min(daily$date), units = "weeks"))
cat("Number of weeks covered:", num_weeks, "\n\n")

# Defining the consumption variables
consumption_vars <- c("consumption_small", "consumption_industry", "consumption_power")

# Function to get first and last non-NA values using pull()
get_first_last_valid <- function(data, column_name) {
  non_na_values <- data %>%
    filter(!is.na(.data[[column_name]])) %>%
    arrange(date)  # Ensure ordering by date

  # Extract first and last non-missing values
  first_value <- non_na_values %>%
    slice(1) %>%
    pull(.data[[column_name]])

  last_value <- non_na_values %>%
    slice(n()) %>%
    pull(.data[[column_name]])
...
# Computing percentage change using first and last valid values
percentage_changes <- sapply(consumption_vars, function(var) {
  if (var %in% colnames(daily)) {
    values <- get_first_last_valid(daily, var)
    if (!is.na(values[1]) & !is.na(values[2])) {
      return(((values[2] - values[1]) / values[1]) * 100)
    } else {
      return(NA)  
    }
  } else {
...
# Printing the percentage changes for each consumption variable
cat("Percentage change in the consumption variables:\n")
print(percentage_changes)
cat("\n") 


# Computing proportion of days marked as holidays
holiday_proportion <- mean(daily$holiday, na.rm = TRUE)
...
# Calculating median consumption for each year-month
...
# Getting the year-month with the lowest median
lowest_median_month <- monthly_medians %>%
  slice(1) %>%
  pull(year_month)

lowest_median_value <- monthly_medians %>%
  slice(1) %>%
  pull(median_consumption_power)

# Print result
cat("The year-month combination with the lowest median consumption_power value is:",
    lowest_median_month, "with a median of", round(lowest_median_value, 4), "\n\n")
